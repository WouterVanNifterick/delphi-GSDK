...
uses
  ...
  {$ENDIF}
  Classes,
  Variants; // so that writeln() can accept multiple types easily
...
type
  ...
  TWinColor= (
    ...
    colForegroundBlue     = $01,
    colForegroundGreen    = $02,
    colForegroundRed      = $04,
    colForegroundYellow   = $06,
    colForegroundIntensity= $08,
...
type
  ...
  TConsoleKey = (A=$41, Add=$6b, Applications=$5d, Attention=$f6, B=$42, Backspace=8, BrowserBack=$a6, BrowserFavorites=$ab, BrowserForward=$a7, BrowserHome=$ac, BrowserRefresh=$a8, BrowserSearch=170, BrowserStop=$a9, C=$43, Clear=12, CrSel=$f7, D=$44, D0=$30, D1=$31, D2=50, D3=$33, D4=$34, D5=$35, D6=$36, D7=$37, D8=$38, D9=$39, Decimal=110, Delete=$2e, Divide=$6f, DownArrow=40, E=$45, &End=$23, Enter=13, EraseEndOfFile=$f9, Escape=$1b, Execute=$2b, ExSel=$f8, F=70, F1=$70, F10=$79, F11=$7a, F12=$7b, F13=$7c, F14=$7d, F15=$7e, F16=$7f, F17=$80, F18=$81, F19=130, F2=$71, F20=$83, F21=$84, F22=$85, F23=$86, F24=$87, F3=$72, F4=$73, F5=$74, F6=$75, F7=$76, F8=$77, F9=120, G=$47, H=$48, Help=$2f, Home=$24, I=$49, Insert=$2d, J=$4a, K=$4b, L=$4c, LaunchApp1=$b6, LaunchApp2=$b7, LaunchMail=180, LaunchMediaSelect=$b5, LeftArrow=$25, LeftWindows=$5b, M=$4d, MediaNext=$b0, MediaPlay=$b3, MediaPrevious=$b1, MediaStop=$b2, Multiply=$6a, N=$4e, NoName=$fc, NumPad0=$60, NumPad1=$61, NumPad2=$62, NumPad3=$63, NumPad4=100, NumPad5=$65, NumPad6=$66, NumPad7=$67, NumPad8=$68, NumPad9=$69, O=$4f, Oem1=$ba, Oem102=$e2, Oem2=$bf, Oem3=$c0, Oem4=$db, Oem5=220, Oem6=$dd, Oem7=$de, Oem8=$df, OemClear=$fe, OemComma=$bc, OemMinus=$bd, OemPeriod=190, OemPlus=$bb, P=80, Pa1=$fd, Packet=$e7, PageDown=$22, PageUp=$21, PauseKey=$13, Play=250, Print=$2a, PrintScreen=$2c, Process=$e5, Q=$51, R=$52, RightArrow=$27, RightWindows=$5c, S=$53, Select=$29, Separator=$6c, Sleep=$5f, Spacebar=$20, Subtract=$6d, T=$54, Tab=9, U=$55, UpArrow=$26, V=$56, VolumeDown=$ae, VolumeMute=$ad, VolumeUp=$af, W=$57, X=$58, Y=$59, Z=90, Zoom=$fb);
...
type
  ...
  TFileAccess  = (Read=1, ReadWrite=3, Write=2);
  ...
  public
    ...
    class function GetStandardFile(stdHandleName: Integer; access: TFileAccess; bufferSize: Integer): TStream; static;
    ...
    class function GetBufferInfo:WinApi.Windows.CONSOLE_SCREEN_BUFFER_INFO;static;
...
procedure ShowDebug(const aString: string); overload;
procedure ShowDebug(const aString: string; Values: array of const ); overload;
procedure ShowInfo(const aString: string); overload;
procedure ShowInfo(const x: Extended); overload;
...
{$IFNDEF CONSOLE}
procedure ConsoleTextOut(canvas: TCanvas; x,y: integer; S: string);
{$ENDIF}
...
resourcestring
  strArgumentOutOfRange_NeedPosNum    = 'ArgumentOutOfRange_NeedPosNum';
  strArgumentOutOfRange_BeepFrequency = 'ArgumentOutOfRange_BeepFrequency';
  StrNotYetImplemented                = 'Not yet implemented.';
...
begin
  if not GetConsoleScreenBufferInfo(ConsoleOutputHandle,info) then
    Exit;
...
{$IFDEF CONSOLE}
var pos:TCoord;len:Integer;NumWritten:Cardinal;
{$ENDIF}
begin
{$IFDEF CONSOLE}
  if IsOutputRedirected then
    Exit;
...
begin
  ...
  begin
    ...
    begin
      case ReadPtr^ of
        #7:
          WinApi.Windows.Beep(800, 200); // this is what my internal speaker uses.
        #8:
          begin
            Dec(WritePtr);
            Inc(DistanceToEdge);
...
begin
  ...
  begin
    ...
    WinApi.Windows.Beep(CurrentFrequency, Millisecs);
...
begin
  WinApi.Windows.Beep(CurrentFrequency, 0);
  SoundState := TSoundState.ssFreed;
end;
...
class function TConsole.GetStandardFile(stdHandleName: Integer;  access: TFileAccess; bufferSize: Integer): TStream;
begin
  Result := THandleStream.Create(stdHandleName);
end;
...
procedure ShowDebug(const aString: string); overload;
begin
  WriteLnC('^G~ZDEBUG:   ^Y' + aString + '^w~z');
end;

procedure ShowDebug(const aString: string; Values: array of const ); overload;
begin
  ShowDebug(Format(aString, Values));
end;
...
procedure ShowInfo(const x: Extended); overload;
begin
  WriteLnC('^B~bINFO:    ^W' + FloatToStr(x) + '^w~z');
end;
...
begin
  ...
  for tmp in Keywords do    Result := ReplaceStr(Result, tmp, '^C' + uppercase(tmp) + '^w');
  for tmp in Keywords do    Result := ReplaceStr(Result, uppercase(tmp), '^C' + uppercase(tmp) + '^w');
  for tmp in Special  do    Result := ReplaceStr(Result, tmp, '^W' + uppercase(tmp) + '^w');
  for tmp in Special  do    Result := ReplaceStr(Result, uppercase(tmp), '^W' + uppercase(tmp) + '^w');
end;
...
begin
  ...
  begin
    ...
    begin
      if CharInSet(Result[I], ['w']) then
        Result[I] := 'Z'
      else if CharInSet(Result[I], ['Z', 'B', 'G', 'C', 'R', 'M', 'Y', 'W']) then
        Result[I] := Lowercase(Result[I])[1];

      if CharInSet(Result[I - 1], ['~']) then
        if CharInSet(Result[I], ['z', 'b', 'g', 'c', 'r', 'm', 'y', 'w']) then
          Result[I] := 'z';
    end;
  end;
end;
...
begin
  ...
  for i := 2 to Length(result) do
  begin
    if (i>2) and (not (CharInSet(Result[i-2] , ['^','~']))) then
      if CharInSet(Result[i-1],['^','~']) then
      begin
        if CharInSet(Result[I],['Z','B','G','C','R','M','Y','W']) then
...
begin
  if (frequency < $25) or (frequency > $7fff) then
      raise EArgumentOutOfRangeException.CreateFmt('frequency', [frequency, strArgumentOutOfRange_BeepFrequency]);
  if (duration <= 0) then
      raise EArgumentOutOfRangeException.CreateFmt('duration', [duration, strArgumentOutOfRange_NeedPosNum]);
...
{var
  ...
  nLength : Integer;
  pNumCharsWritten : DWORD;
  consoleOutputHandle:INT_PTR;
}
begin
{  consoleOutputHandle := self.ConsoleOutputHandle;
//  if (consoleOutputHandle = Windows.INVALID_HANDLE_VALUE) then
//    EInOutError.Create('Invalid handle. No Console.');
  bufferInfo := self.GetBufferInfo;
  nLength := (bufferInfo.dwSize.X * bufferInfo.dwSize.Y);
  pNumCharsWritten := 0;

  Win32Check(FillConsoleOutputCharacter(consoleOutputHandle, ' ', nLength, dwWriteCoord, pNumCharsWritten));
  pNumCharsWritten := 0;
  Win32Check(FillConsoleOutputAttribute(consoleOutputHandle, bufferInfo.wAttributes, nLength, dwWriteCoord, pNumCharsWritten));
  Win32Check(SetConsoleCursorPosition(consoleOutputHandle, dwWriteCoord));
}
  ClrScr;
end;
...
begin
  if ConsoleOutputHandle = INVALID_HANDLE_VALUE then
    Exit(0);
...
class function TConsole.GetBufferInfo: _CONSOLE_SCREEN_BUFFER_INFO;
begin
  {$IFDEF CONSOLE}
  if not GetConsoleScreenBufferInfo(ConsoleOutputHandle {STD_OUTPUT_HANDLE},Result) then
...
begin
  Result := 0;
  if Win32Check(GetConsoleCursorInfo(Self.ConsoleOutputHandle, myconsole_cursor_info)) then
...
begin
  Result := False;
  if Win32Check(GetConsoleCursorInfo(ConsoleOutputHandle, lconsole_cursor_info)) then
    Exit(lconsole_cursor_info.bVisible);
end;

function TConsole.GetError: TTextWriter;
begin
  raise ENotImplemented.Create(StrNotYetImplemented);
end;
...
var t: array [0 .. MAX_PATH] of Char;
begin
  WinApi.Windows.GetConsoleTitle(t,SizeOf(title));
  Result := t;
...
begin
  ...
  if (lConsoleInputHandle = INVALID_HANDLE_VALUE) then
      EInOutError.Create('IO.IO_NoConsole');

  mode := 0;
  if (not GetConsoleMode(lConsoleInputHandle, mode)) then
    RaiseLastOSError;
  begin
      Result := ((mode and 1) = 0);
      exit
...
begin
  GetWindowPlacement( ConsoleOutputHandle, @wp );
...
//        UIPermission.Create(UIPermissionWindow.SafeTopLevelWindows).Demand;
        ...
        if (not ReadConsoleOutput(TConsole.ConsoleOutputHandle, @char_infoRef, dwSize, bufferCoord, readRegion)) then
          RaiseLastOSError;
        ...
        begin
          dwWriteCoord.Y := (i);
          if (not FillConsoleOutputCharacter(ConsoleOutputHandle, sourceChar, sourceWidth, dwWriteCoord, num2)) then
            RaiseLastOSError;
          if (not FillConsoleOutputAttribute(ConsoleOutputHandle, Word(wColorAttribute), sourceWidth, dwWriteCoord, num2)) then
            RaiseLastOSError;
          inc(i)
        end;
        ...
        Win32Check(WriteConsoleOutput(ConsoleOutputHandle, @char_infoRef2, dwSize, bufferCoord, writeRegion))
...
begin
  Result := TConsole.OpenStandardError($100);
end;

class function TConsole.OpenStandardError(bufferSize: Integer): TStream;
begin
  if (bufferSize < 0) then
    raise EArgumentOutOfRangeException.Create('bufferSize ' + 'ArgumentOutOfRange_NeedNonNegNum');

  Result := TConsole.GetStandardFile(GetStdHandle(STD_ERROR_HANDLE), TFileAccess.Write, bufferSize);
end;
...
begin
  ...
  Result := TConsole.GetStandardFile(GetStdHandle(STD_INPUT_HANDLE), TFileAccess.Write, bufferSize);
end;

class function TConsole.OpenStandardInput: TStream;
begin
  Result := TConsole.OpenStandardInput($100);
end;

class function TConsole.OpenStandardOutput: TStream;
begin
  Result := TConsole.OpenStandardOutput($100);
end;
...
begin
  ...
  Result := TConsole.GetStandardFile(GetStdHandle(STD_OUTPUT_HANDLE), TFileAccess.Write, bufferSize);
end;
...
begin
  Console.TextColor(Console.GetBufferInfo.wAttributes and $ff);
end;

procedure TConsole.SetBackgroundColor(value: TConsoleColor);
begin
  raise Exception.Create(StrNotYetImplemented);
end;
...
begin
   if ((AValue < 1) or (AValue > 100)) then
    ...
    //    Avalue := $63;
    // consoleOutputHandle := self.ConsoleOutputHandle;
    if (not GetConsoleCursorInfo(consoleOutputHandle, myConsole_cursor_info)) then
      RaiseLastOSError;
    myConsole_cursor_info.dwSize := Avalue;
    if (not SetConsoleCursorInfo(consoleOutputHandle, myConsole_cursor_info)) then
      RaiseLastOSError;
end;
...
begin
  LastMode := TextAttr;
  TextAttr := byte(TextAttr and $F0) or byte(ord(aValue) and $0F);
  SetConsoleTextAttribute(StdOut, TextAttr);
end;
...
//    UIPermission.Create(UIPermissionWindow.SafeTopLevelWindows).Demand;
  ...
  TThread.Synchronize(TThread.CurrentThread,
  procedure begin
    if (not SetConsoleTitle(PWideChar(AValue))) then
        RaiseLastOSError
  end);
end;
...
begin
  GetWindowPlacement( ConsoleOutputHandle, @wp );
  wp.rcNormalPosition.Left := AValue;
...
//  Exit;
  ...
  if (flag2 and not SetConsoleScreenBufferSize(TConsole.ConsoleOutputHandle, size)) then
    RaiseLastOsError;
  srWindow := bufferInfo.srWindow;
  srWindow.Bottom := (srWindow.Top + height) - 1;
  srWindow.Right := (srWindow.Left + width) - 1;
  if (not SetConsoleWindowInfo(TConsole.ConsoleOutputHandle, true, srWindow)) then
  begin
      // errorCode := GetLastError;
      if (flag2) then
          SetConsoleScreenBufferSize(TConsole.ConsoleOutputHandle, bufferInfo.dwSize);
      largestConsoleWindowSize := GetLargestConsoleWindowSize(TConsole.ConsoleOutputHandle);
...
begin
  GetWindowPlacement( ConsoleOutputHandle, wp );
  wp.rcNormalPosition.Top := AValue;
  SetWindowPlacement( ConsoleOutputHandle,wp);
end;
...
begin
  System.WriteLn(Format(Value,Args));
end;
...
begin
  ...
  Write(TBoxCharactersDouble.UpLeftChar);
  for i := 1 to Len+2 do
    Write(TBoxCharactersDouble.HorChar);
  Write(TBoxCharactersDouble.UpRightChar);
  Write(#13#10);


  Write(TBoxCharactersDouble.VertChar);
  Write(TBoxCharactersDouble.SpaceChar);
  WriteC(S+Col);
  Write(TBoxCharactersDouble.SpaceChar);
  Write(TBoxCharactersDouble.VertChar);
  Write(#13#10);

  Write(TBoxCharactersDouble.LowLeftChar);
  for i := 1 to Len+2 do
    Write(TBoxCharactersDouble.HorChar);
  Write(TBoxCharactersDouble.LowRightChar);
  Write(#13#10);
  WriteC('~z^w');
end;
...
{$IFNDEF CONSOLE}
procedure CanvasWriteLn(canvas: TCanvas; x,y: integer; S: string);
const
  FGEscapeChar                        = '^';
  BGEscapeChar                        = '~';
var
  ...
  procedure WriteText(const T:String);
  begin
    Canvas.TextOut(X,Y,T);
    X := X + canvas.TextWidth(T)+2;
  end;
  procedure SaveState;
  begin
  ...
  function LetterToColor(c:Char):TColor;
  const ColorChars: array [0 .. 15] of Char = 'zbgcrmywZBGCRMYW';
  begin
     p := Pos(c, ColorChars) - 1;
     Result := TConsoleColors.Colors[p]
  end;
  ...
  begin
      case S[I] of
        FGEscapeChar: begin
          Inc(I);
          if I < len then
            Canvas.Font.Color := LetterToColor(S[I]);
        end;
        BGEscapeChar: begin
          Inc(I);
          if I < len then
            Canvas.Brush.Color := LetterToColor(S[I]);
        end;
        else
          WriteText(S[I])
      end;
...
begin
  ...
  try
    ...
    for line := 0 to sl.Count-1 do
      CanvasWriteLn(canvas, x, y + line*abs(Canvas.Font.Height), sl[line] );
